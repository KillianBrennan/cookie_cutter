...
def main(cookie_dir, subdomain_dir):
    ...
    subdomains = xr.open_dataset(
        "/home/kbrennan/phd/data/climate/grids/subdomains_rot.nc"
    )
    ...
    for k in keys:
        os.makedirs(os.path.join(cookie_dir, k), exist_ok=True)
    ...
    for f in cookie_files:
        ...
        for dom in subdomains.keys():
            test = subdomains[dom].sel(
                rlon=rlon,
                rlat=rlat,
                method="nearest",
            ).values
            if test == 1.0:
                shutil.copy(os.path.join(cookie_dir, f), os.path.join(cookie_dir, dom, f))
